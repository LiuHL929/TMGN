...
def config():
    exp_name = "vilt"
    seed = 0
    datasets = ["coco", "vg", "sbu", "gcc","rsvqa"]
    ...
    num_gpus = 1
    num_nodes = 1
    load_path = "./vilt_200k_mlm_itm.ckpt"
    num_workers = 4
    precision = 16
...
@ex.named_config
def task_finetune_rsvqa():
    exp_name = "finetune_rsvqa"
    datasets = ["rsvqa"]
    loss_names = _loss_names({"vqa": 1})
    batch_size = 256
    max_epoch = 10
...
@ex.named_config
def task_finetune_rsvqa_randaug():
    exp_name = "finetune_rsvqa_randaug"
    datasets = ["vqa"]
    train_transform_keys = ["pixelbert_randaug"]
    loss_names = _loss_names({"vqa": 1})
    batch_size = 256
